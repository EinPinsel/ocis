Feature: check activities
  ...
  Scenario: check activities of destination resources after copying a 0 byte file
    Given user "Alice" has uploaded file "filesForUpload/zerobyte.txt" to "/zerobyte.txt"
    And user "Alice" has created folder "/FOLDER"
    And user "Alice" has copied file "zerobyte.txt" to "FOLDER/zerobyte.txt"
    When user "Alice" lists the activities of file "FOLDER/zerobyte.txt" from space "Personal" using the Graph API
    Then the HTTP status code should be "200"
    And the JSON data of the response should match
      """
  ...
  @issue-10153
  Scenario: check all activities without filtering by resourceId
    Given user "Alice" has uploaded file with content "ownCloud test text file 0" to "/textfile.txt"
    And user "Alice" has created folder "/FOLDER"
    When user "Alice" lists all activities using the Graph API without specifying any resourceId
    Then the HTTP status code should be "200"
    And the JSON data of the response should match
      """
      {
